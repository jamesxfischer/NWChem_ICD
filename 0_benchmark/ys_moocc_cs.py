...
outfile = glob.glob('Ar-rCAM-def2TZVPPD-Gau0.4-t45p45-frq1.15-gamma1.0-zeta1.0-E0.07-dt0.5.nwo')
...
#	c=a+b
	np.savetxt(filename+'.moocc',a , fmt="%f")

occfiles = glob.glob('Ar-rCAM-def2TZVPPD-Gau0.4-t45p45-frq1.15-gamma1.0-zeta1.0-E0.07-dt0.5.moocc')
for ifile in occfiles :
	filename=ifile.strip(".moocc")
	occf=pd.read_csv(ifile ,header=None,delimiter=' ')
	f=pd.DataFrame(occf)
	f2=f.drop(columns=0)
	f_plot=f2.loc[0:100,5:13]
