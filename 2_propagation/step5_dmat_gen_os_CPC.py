...
#################################################################
#JZ 1/11/2023 add in dynamic pulling of mo_numbers from rt_restart file
#################################################################
#find out how many atomic orbitals there are, store in nbf_ao

with open('gamma1.0-zeta0.7-E0.01-dis3.3.rt_restart', 'r+') as f:
	lines = f.readlines()
	for line in lines:
		if 'nbf_ao' in line:
			line = line.strip('\n')
...
f.close()
#############################################################################
#JZ 1/11/2023 add in dynamic pulling of mo_numbers from rt_restart file END
##############################################################################
...
## 2.1 Pao= C.Pmo.C+, C is movecs   ??? try os_moves later
## YS: How to ignore the useless lines in movec files?
##########################################################################
## JZ 1/10/2023 add in loop to dynamically grab C_file name for Cmo ##
##########################################################################
nw_files=glob.glob("*.nw")
x=0
while x < len(nw_files):
	filename=nw_files[x].strip(".nw")
	C_file=open(filename+'.cmo',"r")
###########################################################################
## JZ 1/10/2023 add in loop to dynamically grab C_file name for Cmo -END ##
###########################################################################
